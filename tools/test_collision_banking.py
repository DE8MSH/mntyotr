...
def main():
    ...
    tail4 = (ROOT / 'src/room04_assets_tail.asm').read_text()
    ...
    assert 'bcc     .select_room' in bank
    ...
    assert 'room04_collision_map_rom:' in tail4

    assert 'tma3' in bank and 'tma4' in bank
    assert 'tam3' in bank and 'tam4' in bank
    assert 'sei' in bank and 'cli' in bank

    # Sensitive physics remains direct-pointer based and unchanged; Rooms03/04
    # are adapted outside it through the shared Room02 RAM cache.
    ...
    assert 'room04_collision_map' not in physics

    print('OK: Room00/01 ROM banking + shared Room02/03/04 RAM collision cache')
